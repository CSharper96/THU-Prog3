...
public:
    ...
    {
        const char *fn = filename; // read image...
        data = stbi_load(fn, &w, &h, &c, 1);
        std::cout << fn << ":" << w << "x" << h << " with channels: " << c << "\n";
    }
    ...
    {
        stbi_write_png(image, w, h, 1, data, w);
    }
    ...
    //Ein Image Objekt soll es erlauben, die Bildgroesse abzufragen ("getWidth", "getHeight").
    ...
    //Schein 3, Aufgabe 3 - copy und move
    //Ergänzen Sie die Image-Klasse aus der „Übung 2“, so dass man ein Bild kopieren und verschieben
    //kann. Achten Sie dabei auf das korrekte Verhalten:

    //-Beim Kopieren muss der Bildinhalt (die Pixel) kopiert werden (d.h., jedes Pixel muss
    //kopiert werden).
    //Copy-Konstruktor
    Image(const Image &other) : h{other.h}, w{other.h}, c{other.c}, data{new stbi_uc{}}
    {
        *data = *(other.data);
    }
    //Copy-Operator
    ...
    //-Beim Verschieben soll der Bildinhalt (auf den der Pointer auf die Pixel zeigt) „geklaut“
    //werden.
    //Move-Konstruktor
    Image(Image &&other) : h{other.h}, w{other.h}, c{other.c}, data{other.data}
    {
        other.data = nullptr;
    }
    //Move-Operator
